...
checks with integrity pinning. The list is still intentionally not an invocation
authority; a future executable host must consume this same helper before
exposing any plugin tool.

Suite: 618 tests across 64 files, all passing, with `verify` covering format,
