...
Latest evidence: `npm run verify` (63 files, 606 tests),
...
Suite: 606 tests across 63 files, all passing, with `verify` covering format,
...
- CI now pins third-party action SHAs and emits an unsigned commit/artifact provenance manifest; the latest local provenance evidence covers 42 build artifacts; signed OIDC attestation remains a release-owner gate.
