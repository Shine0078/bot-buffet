...
Latest evidence: `npm run verify` (64 files, 613 tests),
...
The list is still intentionally not an invocation authority; a future plugin
runtime must consume this same helper before exposing any plugin tool.

Suite: 613 tests across 64 files, all passing, with `verify` covering format,
...
including the schedule dispatcher and expiry-pruning changes are covered by the
local evidence above but are not part of that sealed snapshot. No replacement
