...
Latest evidence: `npm run verify` (64 files, 628 tests),
...
Suite: 628 tests across 64 files, all passing, with `verify` covering format,
...
- Explicitly expired memory records can be pruned through an authorization-filtered, namespace-scoped, 1,000-record bounded route; each deletion uses the observed entity version, deletion counts are audited, and policy-based read retention never becomes an implicit delete.
...
- Safe project deletion now requires the current project version and runs as one serialized store mutation with a persistent scope tombstone, removes child run state, revokes credential `secretRef` values, preserves audit events, and rejects delayed child writes; schedules remain disabled and webhook metadata is HTTPS/secret-protected.
...
- Project updates/deletion, provider health/delete operations, source retrieval, memory deletion/pruning, plugin cleanup, and evaluation-case dataset appends now require current-version CAS tokens; destructive mutation audits identify accepted changes, and a failed dataset CAS removes the newly inserted case instead of leaving an orphan.
