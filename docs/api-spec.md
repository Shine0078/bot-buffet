...
Provider health tests and administrative deletes require the current provider `version` and use compare-and-swap persistence. Successful health tests emit `provider.tested`; deletes disable the provider and credential, revoke the vault secret, and emit a critical `provider.disabled` audit event. Stale requests fail with `provider_version_required` before the provider or vault changes.
