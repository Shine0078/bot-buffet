...
Core resources include projects, providers, models, memory, plugins, MCP servers, files, sources, schedules, webhooks, evaluation datasets/cases, runs, approvals, audit, and observability. Projects can be archived or safely deleted after active runs are stopped; deletion requires the current project version, removes the project and durable children in one serialized store mutation, tombstones the project scope against delayed child writes, revokes child credentials, and preserves audit events. Run creation accepts an `Idempotency-Key`; the key is hashed and the 202 response is durably replayable for 24 hours. SSE requires a project scope in production, checks project authorization, caps subscribers, and emits heartbeats.
