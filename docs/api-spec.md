...
Core resources include projects, providers, models, memory, plugins, MCP servers, files, sources, evaluation datasets/cases, runs, approvals, audit, and observability. Run creation accepts an `Idempotency-Key`; the key is hashed and the 202 response is durably replayable for 24 hours. SSE requires a project scope in production, checks project authorization, caps subscribers, and emits heartbeats.
