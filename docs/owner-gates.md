...
- Security owner: keep `BOT_BUFFET_SANDBOX_MODE=docker` (or an equivalent microVM runner) in every environment, make `docker info`/the configured runner reachable before startup, validate the pinned provider socket transport with DNS-rebinding tests, and provide a kernel/container sandbox escape report. The host-process fallback has been removed because it cannot provide a portable ancestor no-follow guarantee; do not grant the control plane an unreviewed privileged Docker socket.
